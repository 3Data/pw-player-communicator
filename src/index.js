...
class Communicator extends EventEmitter {
  ...
  onMessageReceived = (e) => {
    switch (e.data.type) {
      ...
      // Chat events:
      case "onChatMessage":
        this.emit("chatMessage", e.data.data);
        break;
      case "onRoomModeUpdate":
        this.emit("roomModeUpdate", e.data.roomMode);
        break;
      case "onVIPRequestStatusUpdate":
        this.emit("VIPRequestStatusUpdate", e.data.status);
        break;
      // Common events:
      case "onDisconnected":
        this.emit("disconnected", e.data.reason);
        break;
  ...
  stopVideo = () =>
    sendMessageToVideo({ type: "stopVideo", sessionToken: this.sessionToken });

  pauseVideo = () =>
    sendMessageToVideo({ type: "pauseVideo", sessionToken: this.sessionToken });

  playVideo = () =>
    sendMessageToVideo({ type: "playVideo", sessionToken: this.sessionToken });
